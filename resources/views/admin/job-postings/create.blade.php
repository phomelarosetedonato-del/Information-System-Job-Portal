...
@section('title', 'Create Job Posting - Admin Panel')
...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ route('admin.job-postings.index') }}" class="d-none d-sm-inline-block btn btn-sm btn-secondary shadow-sm">
        ...
        <div class="col-lg-12">
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">Job Posting Details</h6>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.job-postings.store') }}" id="jobPostingForm">
                        @csrf
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('job_category') is-invalid @enderror"
                                        ...
                                        @foreach($jobCategories as $key => $value)
                                            <option value="{{ $key }}" {{ old('job_category') == $key ? 'selected' : '' }}>
                                                {{ $value }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('experience_level') is-invalid @enderror"
                                        ...
                                        @foreach($experienceLevels as $key => $value)
                                            <option value="{{ $key }}" {{ old('experience_level') == $key ? 'selected' : '' }}>
                                                {{ $value }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="card mt-3 p-3 border">
                            ...
                            <div class="form-check mb-2">
                                <input type="checkbox" class="form-check-input" id="is_remote" name="is_remote" value="1" {{ old('is_remote') ? 'checked' : '' }}>
                                <label class="form-check-label" for="is_remote">Is this role remote?</label>
                            </div>

                            <div class="form-check mb-2">
                                <input type="checkbox" class="form-check-input" id="provides_accommodations" name="provides_accommodations" value="1" {{ old('provides_accommodations') ? 'checked' : '' }}>
                            ...
                            <div class="form-group mt-2">
                                ...
                                <textarea id="accessibility_features" name="accessibility_features" class="form-control" rows="3">{{ old('accessibility_features') }}</textarea>
                            ...
                            <div class="form-group mt-2">
                                ...
                                <input type="text" id="assistive_technology" name="assistive_technology" class="form-control" value="{{ old('assistive_technology') }}" placeholder="e.g. JAWS, NVDA, speech-to-text tools">
                            </div>
                                    ...
                                    @foreach($disabilityTypes as $dt)
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="disability_type_ids[]" value="{{ $dt->id }}" id="dt-{{ $dt->id }}"
                                                {{ in_array($dt->id, old('disability_type_ids', [])) ? 'checked' : '' }}>
                        ...
                        <div class="row mt-4">
                            <div class="col-md-6">
                                <div class="form-check">
                                    ...
                                    <small class="form-text text-muted d-block">Uncheck to create as inactive/draft</small>
                                </div>
                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Auto-format salary display
    function formatSalaryDisplay() {
        const min = salaryMin.value ? parseFloat(salaryMin.value).toLocaleString() : '';
        const max = salaryMax.value ? parseFloat(salaryMax.value).toLocaleString() : '';

        if (min && max) {
            document.getElementById('salaryDisplay').textContent = `₱${min} - ₱${max}`;
        } else if (min) {
            document.getElementById('salaryDisplay').textContent = `₱${min}+`;
        } else if (max) {
            document.getElementById('salaryDisplay').textContent = `Up to ₱${max}`;
        } else {
            document.getElementById('salaryDisplay').textContent = 'Not specified';
        }
    }

    salaryMin.addEventListener('input', formatSalaryDisplay);
    salaryMax.addEventListener('input', formatSalaryDisplay);
});
</script>
@endsection

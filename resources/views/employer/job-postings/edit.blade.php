...
<div class="row">
    <div class="col-12 mb-4">
        <h2 class="fw-bold">
            <i class="fas fa-edit text-primary"></i> Edit Job Posting
        </h2>
    </div>
    <div class="col-12 col-md-12 col-lg-10 mx-auto">
        <div class="card shadow-sm">
            <div class="card-body">
                <form method="POST" action="{{ route('employer.job-postings.update', $job->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        ...
                        <div class="col-12 col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control @error('salary') is-invalid @enderror" id="salary" name="salary" value="{{ old('salary', $job->salary) }}" min="0">
                                    ...
                                    <div class="col-12 col-md-4 mb-2">
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="disability_type_ids[]" value="{{ $type->id }}" id="disability_type_{{ $type->id }}"
                                                {{ (is_array(old('disability_type_ids', $job->suitableDisabilityTypes->pluck('id')->toArray())) && in_array($type->id, old('disability_type_ids', $job->suitableDisabilityTypes->pluck('id')->toArray()))) ? 'checked' : '' }}>
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <textarea class="form-control @error('requirements') is-invalid @enderror" id="requirements" name="requirements" rows="4" required>{{ old('requirements', $job->requirements) }}</textarea>
                    ...
                    <button type="submit" class="btn btn-primary"><i class="fas fa-save me-1"></i> Update Job Posting</button>
                </form>
            </div>
        </div>
    </div>
